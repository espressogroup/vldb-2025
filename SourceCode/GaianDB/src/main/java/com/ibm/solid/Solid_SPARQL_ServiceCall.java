...
public class Solid_SPARQL_ServiceCall {
  ...
  public class SearchResultScen5 {
    ...
    try {
        ...
        writeResultsToCSV(Arrays.asList(searchResultsScen5), csvFilePath,sparqlquery);
...
//            csvWriter.writeNext(header);
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                writeResultsToCSV(Arrays.asList(searchResults), csvFilePath,sparqlquery);
...
private void writeResultsToCSV(List<SearchResultScen5> searchResults, String csvFilePath, String sparqlQuery) throws IOException {
    Field[] fields = SearchResultScen5.class.getDeclaredFields();
    List<String> validFieldNames = new ArrayList<>();
    for (Field field : fields) {
        if (!field.isSynthetic()) {
            validFieldNames.add(field.getName().toUpperCase());
        }
    }

    // Add SPARQL query as an additional column header
    validFieldNames.add("SPARQL");

    String[] headers = validFieldNames.toArray(new String[0]);
        ...
        for (SearchResultScen5 result : searchResults) {
            ...
            for (Field field : fields) {
                if (!field.isSynthetic()) {
                    ...
                    valueStr = URLDecoder.decode(valueStr, StandardCharsets.UTF_8.name());
                    rowValues.add(valueStr);
                }
            }

            // Add the SPARQL query value to each row
            String decodedSparqlQuery = URLDecoder.decode(sparqlQuery, StandardCharsets.UTF_8.name());
            rowValues.add(decodedSparqlQuery);

            csvWriter.writeNext(rowValues.toArray(new String[0]), false);
        }
    } catch (IllegalAccessException e) {
        throw new RuntimeException("Error accessing SearchResult fields", e);
    }
