					...
					if (latestLastModifiedTime != fileLastModified) {
						...
						if (null == result) {
							try {
								...
								String originalSQLComment = (String) arguments.get(GaianTable.QRY_COMMENT);

								logger.logAlways("VTIFile original-sql: " + originalSQL);

								System.out.println("VTIFile original-sql<2>: " + originalSQL);
								System.out.println("VTIFile original-sql comment<1>: " + originalSQLComment);
								System.out.println("VTIFile original-sql both<1>: " + originalSQL+" "+originalSQLComment);
				...
				if (null == result) {
					...
					if (isInMemRows)
						...
						try {
							...
							System.out.println("VTIFile original-sql<2>: " + originalSQL);
							System.out.println("VTIFile original-sql comment<2>: " + originalSQLComment);
							System.out.println("VTIFile original-sql both<2>: " + originalSQL+" "+originalSQLComment);
