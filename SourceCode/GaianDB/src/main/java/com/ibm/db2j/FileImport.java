...
//    	if ( ucs.endsWith(".ZIP") || ucs.endsWith(".GZIP") ) s = s.substring(0, s.lastIndexOf('.'));
	...
	public FileImport(String s, String sqlQuery) throws Exception {
		...
		try {
			...
			System.out.println("Original Query(Could be Orig or Passed): "+ originalSqlQuery);
			...
			if (hasGaianTable(sqlQuery)) {
				...
				System.out.println("Query When Passed From neigbours???"+originalSqlQuery);
			}
...
//			String sparqlQuery= new SqlParser().extractSparqlQuery(originalSqlQuery);
			...
			String sparqlQuery =new SqlParser().extractSparqlQueryFromWhereClause(originalSqlQuery);
	...
	public static Map<String,String> parseGaianTableQuery(String queryString) {
		...
		if (matcher.find()) {
			...
			System.out.println("LT_Table_Name: " + tableName);
		}
...
//		}
		...
		System.out.println("I Don't Think this is reached!!");
