...
public class GaianTable extends AbstractVTI implements VTICosting, IQualifyable, Pushable, IFastPath {
	...
	public static final String QRY_COMMENT="qry_comment";
...
//		if ( latestInstanceConfigLoadTime < DataSourcesManager.getLatestGlobalConfigLoadTime() ) {
		...
		/*******Ragab_Debug if is PropagatedQuery********/
		else {
...
//						if ( false == isLogPerfOn && null != perfFileBW ) { perfFileBW.close(); perfFileBW = null; }
		...
		if ( 0 == queryPropagationCount ) {
			System.out.println("Context:::"+getContext()+"NEW INCOMING SQL QUERY::::" + originalSQL);
...
//	private static final Pattern sparqlPattern = Pattern.compile("(?is)-- SPARQL Query: --.*");

	private static final Pattern wherePattern = Pattern.compile("(?i)WHERE(.*)");
...
//			System.out.println("Contents of sqls1::::");
//			for (String sql : sqls1) {
//				System.out.println(sql);
//				}
//
//			System.out.println("Contents of sqls2:::::");
...
//				}
		...
		if ( 2 == sqls1.length && 2 == ( sqls2 = wherePattern.split( sqls1[1] ) ).length ) {
			...
			System.out.println("FROM in GAIANTABLE:::"+fromSQL);
			System.out.println("sqlAfterWHERE in GAIANTABLE:::"+sqlAfterWhereToken);
...
//    				0 > fromSQL.indexOf(',') && !joinPattern.matcher(fromSQL).matches() ) { // avoid all possible joins if it is not a VTI call
		...
		System.out.println("ORIGNAL QUERY in GAIANTABLE:::::::"+originalSQL);
		int sparqlIndex = originalSQL.indexOf("-- SPARQL Query:");
        if (sparqlIndex != -1) {
            // Add the length of the keyword to get the start of the actual query
            sparqlIndex += "-- SPARQL Query:".length();

            // Extract the substring from the original string starting from the calculated index
            String extractedSparqlString = originalSQL.substring(sparqlIndex).trim();

            System.out.println("COMMENT::" +extractedSparqlString);
            queryDetails.put(QRY_COMMENT,extractedSparqlString);
        } else {
            System.out.println("Keyword not found in the string!!!!");
        }
